...
impl Serialize for SimdInstruction {
	...
	fn serialize<W: io::Write>(self, writer: &mut W) -> Result<(), Self::Error> {
		...
		match self {
			V128Const(ref c) => simd_with_other!(writer, V128_CONST, writer.write(&c[..])?),
			V128Load(m) => simd_with_other!(writer, V128_LOAD, MemArg::serialize(m, writer)?),
			V128Store(m) => simd_with_other!(writer, V128_STORE, MemArg::serialize(m, writer)?),
			...
			I8x16ExtractLaneS(i) => simd_with_other!(writer, I8X16_EXTRACT_LANE_S, writer.write(&[i])?),
			I8x16ExtractLaneU(i) => simd_with_other!(writer, I8X16_EXTRACT_LANE_U, writer.write(&[i])?),
			I16x8ExtractLaneS(i) => simd_with_other!(writer, I16X8_EXTRACT_LANE_S, writer.write(&[i])?),
			I16x8ExtractLaneU(i) => simd_with_other!(writer, I16X8_EXTRACT_LANE_U, writer.write(&[i])?),
			I32x4ExtractLane(i) => simd_with_other!(writer, I32X4_EXTRACT_LANE, writer.write(&[i])?),
			I64x2ExtractLane(i) => simd_with_other!(writer, I64X2_EXTRACT_LANE, writer.write(&[i])?),
			F32x4ExtractLane(i) => simd_with_other!(writer, F32X4_EXTRACT_LANE, writer.write(&[i])?),
			F64x2ExtractLane(i) => simd_with_other!(writer, F64X2_EXTRACT_LANE, writer.write(&[i])?),
			I8x16ReplaceLane(i) => simd_with_other!(writer, I8X16_REPLACE_LANE, writer.write(&[i])?),
			I16x8ReplaceLane(i) => simd_with_other!(writer, I16X8_REPLACE_LANE, writer.write(&[i])?),
			I32x4ReplaceLane(i) => simd_with_other!(writer, I32X4_REPLACE_LANE, writer.write(&[i])?),
			I64x2ReplaceLane(i) => simd_with_other!(writer, I64X2_REPLACE_LANE, writer.write(&[i])?),
			F32x4ReplaceLane(i) => simd_with_other!(writer, F32X4_REPLACE_LANE, writer.write(&[i])?),
			F64x2ReplaceLane(i) => simd_with_other!(writer, F64X2_REPLACE_LANE, writer.write(&[i])?),
			V8x16Shuffle(ref i) => simd_with_other!(writer, V8X16_SHUFFLE, writer.write(&i[..])?),
